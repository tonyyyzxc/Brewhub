...
$shop = bh_fetch_seller_profile($conn, $sellerId);

if (trim($shop['shop_name']) === '' || trim($shop['shop_name']) === "'" . $shop['username'] . "'s Shop") {
    // Verify seller actually exists and is a seller
    $chk = $conn->prepare("SELECT role FROM users WHERE user_id = ?");
    $chk->bind_param('i', $sellerId);
    $chk->execute();
    $row = $chk->get_result()->fetch_assoc();
    $chk->close();
    if (!$row || !in_array(strtolower((string)($row['role'] ?? '')), ['seller', 'both', 'admin'], true)) {
        header('Location: Dashboard.php');
        exit;
    }
}
...
<head>
    ...
    <title><?php echo htmlspecialchars($shop['shop_name'], ENT_QUOTES, 'UTF-8'); ?> – Brewhub</title>
...
<body class="dashboard-page">
    ...
    <main class="profile-page-main py-5">
        <div class="container-fluid px-5 pb-5 products-grid">
            <div class="mb-4">
                <p class="profile-kicker mb-1"><i class="bi bi-shop me-2"></i>Shop</p>
                <h1 class="seller-dashboard-title mb-1"><?php echo htmlspecialchars($shop['shop_name'], ENT_QUOTES, 'UTF-8'); ?></h1>
                <?php if ($shop['description'] !== ''): ?>
                    <p class="text-muted mb-0"><?php echo htmlspecialchars($shop['description'], ENT_QUOTES, 'UTF-8'); ?></p>
                <?php endif; ?>
            </div>
            <?php if ($flash): ?>
                        ...
                        <div class="bh-section-card">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <div class="text-muted">This shop hasn't listed any products.</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <?php foreach ($products as $p): ?>
                        ...
                        <div class="col-12 col-sm-6 col-lg-4">
                            <div
                                ...
                                data-shop="<?php echo htmlspecialchars($shop['shop_name'], ENT_QUOTES, 'UTF-8'); ?>"
                            >
